# simple-meridian/run_briefing_fake_news.py
"""Essa cópia só pode ser executada junto com o feed fake_news"""
...
from meridiano.utils import cria_arquivo_veracidade
...
def process_articles(feed_profile, effective_config, limit=1000):
    ...
    for article in unprocessed:
        ...
        """Vamos passar para o processo de embedding o artigo completo"""
...
def rate_articles(feed_profile, effective_config, limit=1000):
    ...
    """ Vamos pegar o conteudo completo para fazer o rate"""
    ...
    for article in unrated:
        ...
        if rating_response:
            ...
            """ --- Minha modificação ---"""
            try:
                match = re.search(r"\b([0-2])\b", rating_response.strip())
                if match:
                    score = int(match.group(1))
                    if 0 <= score <= 2:
                        impact_score = score
                    ...
                    print(
                        f"  Warning: Could not find valid rating (0-2) in response '{rating_response}' "
                        f"for article {article['id']}."
                    )
            except (ValueError, AttributeError) as e:
                print(
        ...
        """ --- Minha modificação ---"""
        if impact_score is not None:
            database.salva_veracidade(article["id"], veracidade_llm=impact_score, feed_profile=feed_profile)
            rated_count += 1
...
def generate_brief(feed_profile, effective_config):
    """Generates the bri    efing for a specific feed profile."""
    ...
    """Vamos salvar esses rótulos no banco de dados para análises futuras"""
    """1 - Vamos associar cada notícia a seu cluster"""
    cluster_analyses = []
    ...
    for i in range(n_clusters):
        ...
        cluster_articles_ids = [article_ids[idx] for idx in cluster_indices]

        for article_id in cluster_articles_ids:
            database.salva_veracidade(article_id, veracidade_kmeans=i, feed_profile=feed_profile)
        

"""   --- Analise dos clusters e sintese em briefing ---
...
""" --- Meu código --- """
""" Este código deve comparar o cluster gerado pelo kmeans com a classificação do LLM.
    Se as respostas forem divergentes, deve ser criada uma tabela informando as divergencias. """
def verifica_respostas():
    print("\n--- Verificando divergências entre KMeans e LLM ---")
    dados = database.get_articles_for_briefing(config.BRIEFING_ARTICLE_LOOKBACK_HOURS, feed_profile="fake_news")
    divergencias = {}
    divergencias_count = 0

    print(f"\nForam encontrados {len(dados)} artigos para análise de divergências.")
    for artigo in dados:
        id_artigo = artigo["id"]
        url = artigo["url"]
        veracidade = artigo["veracidade"]
        veracidade_llm = artigo["veracidade_llm"]
        veracidade_kmeans = artigo["veracidade_kmeans"]
        raw_content = artigo["raw_content"]

        if veracidade_llm is not None and veracidade_kmeans is not None:
            if veracidade_llm != veracidade_kmeans:
                divergencias[id_artigo] = {
                    "veracidade": veracidade,
                    "veracidade_llm": veracidade_llm,
                    "veracidade_kmeans": veracidade_kmeans,
                    "url": url,
                    "conteudo": raw_content,
                }
                divergencias_count += 1
    print(f"\nTotal de divergências encontradas: {divergencias_count}")

    cria_arquivo_veracidade(divergencias)
    print(f"Arquivo 'veracidade_discrepancias.csv' criado com as divergências encontradas.")
...
def main():
    ...
    if should_run_all:
        print("\n>>> Running ALL stages <<<")
        " --- Antes de fazer o processamento, precisamos inserir ao banco de dados uma notícia claramente falsa ou claramente verdadeira. --- "
        # insere_base_de_dados()
        ...
        if args.rate:
            ...
            rate_articles(feed_profile_name, effective_config, limit=args.limit)
            """
            print("Rating stage is currently disabled.")
        if args.generate:
